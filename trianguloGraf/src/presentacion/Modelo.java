...
public class Modelo implements Runnable{
    ...
    public void crearTriangulo(){
        ...
        getMiSistema().setAncho(getVentana().getLienzo().getWidth());
        getMiSistema().setAlto(getVentana().getLienzo().getHeight());
    ...
    protected void dibujar(Graphics lapiz){
        ...
        lapiz.fillRect(0, 0, getMiSistema().getAncho(), getMiSistema().getAlto());   // Limpia la superficie de dibuja con el color de fondo en el área especificada
        lapiz.setColor(new Color(0xff, 0x00, 0x00));
        // puntos
        if(getMiSistema().getCantidadTriangulos() > 0){
            xA = getMiSistema().getTrianguloActual().getxA();
            yA = getMiSistema().getTrianguloActual().getyA();

            xB = getMiSistema().getTrianguloActual().getxB();
            yB = getMiSistema().getTrianguloActual().getyB();

            xC = getMiSistema().getTrianguloActual().getxC();
            yC = getMiSistema().getTrianguloActual().getyC();

            lapiz.drawLine(xA, yA, xB, yB);
            lapiz.drawLine(xB, yB, xC, yC);
            lapiz.drawLine(xA, yA, xC, yC);
        }
    ...
    public void run() {
        ...
        dobleBuffer = new BufferedImage(w, h, BufferedImage.TYPE_INT_ARGB);
